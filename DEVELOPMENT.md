...
from Loom kernels and the HRX runtime HAL. It accepts dynamic structured
prompts, tokenizes with the HRX tokenizer, runs Qwen3-VL conditioning, executes
...
The HRX parameter index supplies checkpoint and baked archive tensors.
...
layout: headers, C sources, tests, benchmarks, and `BUILD.bazel` files live
together.
