...
paths directly to the ordinary HRX file loaders: relative `--flagfile`,
...
Install Bazel 9.1, initialize the submodule, and select the ROCm SDK. The
repository's `.bazelrc` enables the AMDGPU HAL and Loom target:

```bash
git submodule update --init --recursive
export IREE_ROCM_PATH=/opt/rocm
...
unconditioned DiT, and VAE parameter scopes. Both are ordinary HRX flagfiles,
...
```bash
cp docs/ideogram4-fp8.flags /path/to/id4-local.flags
```

The resulting local file has this shape:

```text
--flagfile=docs/ideogram4-fp8-policy.flags
...
```bash
bazel-bin/binding/cli/id4 --list_devices=amdgpu
```
...
requests under `docs/requests/` provide 128, 256, and 512 pixel
...
```bash
bazel-bin/binding/cli/id4 \
  --flagfile=docs/ideogram4-fp8-execution-layout.flags \
  --device=amdgpu:// \
  --prompt_json_file=docs/requests/long_1024.json \
  --output=ideogram4.ppm
```
