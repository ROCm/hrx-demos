...
#include "stages/vae.h"

#include "pipeline/plan.h"
#include "pipeline/stage.h"
#include "stages/test_util.h"
...
static id4_pipeline_stage_t* CreateStage(iree_hal_device_group_t* device_group,
  ...
  IREE_CHECK_OK(
      id4_vae_stage_create(&create_options, iree_allocator_system(), &stage));
  return stage;
}
...
TEST(VaeStage, PlansDecodeFromRequestConfig) {
  ...
  id4_pipeline_stage_plan_options_t missing_vae_options;
  memset(&missing_vae_options, 0, sizeof(missing_vae_options));
  missing_vae_options.structure_size = sizeof(missing_vae_options);
  missing_vae_options.device_index = 0;
  missing_vae_options.queue_affinity = IREE_HAL_QUEUE_AFFINITY_ANY;
  missing_vae_options.diagnostics_sink = &diagnostics_sink;

  id4_pipeline_plan_t* plan = nullptr;
  IREE_EXPECT_STATUS_IS(
      IREE_STATUS_INVALID_ARGUMENT,
      id4_pipeline_stage_plan(stage, &missing_vae_options, &plan));
  ...
  EXPECT_GT(region->statistics.local_slab_high_water_mark, 0u);
...
TEST(VaeStage, RejectsInvalidStaticModelConfig) {
  iree_hal_device_group_t* device_group =
      id4::test::CreateLocalSyncDeviceGroup();
